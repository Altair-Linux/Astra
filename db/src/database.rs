...
/// record of a package that's been installed.
#[derive(Debug, Clone)]
...
impl Database {
    ...
    /// opens an in-memory database (handy for testing).
    ...
    /// records a newly installed package in the database.
    pub fn install_package(
        ...
        tx.execute(
            "DELETE FROM files WHERE package_name = ?1",
    ...
    /// removes a package from the database, returns its file list.
    ...
    /// gets the file list for a package.
    ...
    /// finds which package owns a given file path.
    ...
    /// returns the stored metadata json for a package.
...
mod tests {
    ...
    fn test_install_and_query() {
        ...
        db.install_package(&meta, &files, InstallReason::Explicit)
            .unwrap();
    ...
    fn test_list_and_search() {
        let db = Database::open_memory().unwrap();
        db.install_package(&test_metadata("alpha"), &[], InstallReason::Explicit)
            .unwrap();
        db.install_package(&test_metadata("beta"), &[], InstallReason::Explicit)
            .unwrap();
    ...
    fn test_dependencies() {
        ...
        db.install_package(&meta, &[], InstallReason::Explicit)
            .unwrap();

        // install dependency too
        db.install_package(&test_metadata("lib-a"), &[], InstallReason::Dependency)
            .unwrap();
